namespace HashTable
{
    ...
    public class HashTable<TKey, TValue> : IEnumerable<KeyValue<TKey, TValue>>
    {
        ...
        public bool AddOrReplace(TKey key, TValue value)
        {
            ...
            {
                if (argumentException.Message.Contains("Duplicate Key") && argumentException.ParamName == key.ToString())
                {
                    int index = Math.Abs(key.GetHashCode()) % this.Capacity;

                    var keyValue = this.slots[index].FirstOrDefault(kvp => kvp.Key.Equals(key)); foreach (var element in this.slots[index])
